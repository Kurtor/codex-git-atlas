import { ArrowsClockwise, ChartLineUp, ClockCounterClockwise, Code, GitBranch, GitMerge, Path, TreeStructure, Warning } from '@phosphor-icons/react';
...
export function ModeWorkspace({ mode, data, commits, selected, activeModule, onModule, onSelect }: { mode: AppMode; data: RepositoryData; commits: GitCommit[]; selected: GitCommit; activeModule: string; onModule: (name: string) => void; onSelect: (hash: string) => void }) {
  ...
  const mergeCount = commits.filter((commit) => commit.operations.some((operation) => operation.kind === 'merge')).length;
  const rebaseCount = commits.filter((commit) => commit.operations.some((operation) => operation.kind === 'rebase')).length;
  const metrics = mode === 'history'
    ? [['范围提交', commits.length, '当前筛选范围'], ['本地分支', data.refs.filter((ref) => ref.type === 'local').length, '按可达性计算'], ['分支行为', mergeCount + rebaseCount, `${mergeCount} 合并 + ${rebaseCount} Rebase`], ['代码变更', compactNumber(churn), '新增 + 删除']]
    : mode === 'causal'
