...
function ModulePanel({ commits, activeModule, onModule }: { commits: GitCommit[]; activeModule: string; onModule: (name: string) => void }) {
  ...
  return <div className="mode-detail module-detail"><button aria-pressed={activeModule === '全部模块'} className={activeModule === '全部模块' ? 'active' : ''} onClick={() => onModule('全部模块')}><span>全部模块</span><strong>{modules.length}</strong><small>清除模块筛选</small><i style={{ width: '100%' }} /></button>{modules.map((module) => <button key={module.name} aria-pressed={activeModule === module.name} title={`${module.name} · ${module.commits} 次触达 · 风险 ${module.averageRisk}`} className={activeModule === module.name ? 'active' : ''} onClick={() => onModule(module.name)}><span>{module.name}</span><strong>{compactNumber(module.churn)}</strong><small>{module.commits} 次触达 · 风险 {module.averageRisk}</small><i style={{ width: `${Math.max(10, module.churn / max * 100)}%` }} /></button>)}</div>;
}
