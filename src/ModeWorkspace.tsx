...
export const modeCopy = {
  history: { index: '01', eyebrow: 'HISTORY', title: '提交演化', description: '按时间读取仓库如何一步步变成现在，并在稳定拓扑上定位关键转折。', Icon: ClockCounterClockwise },
  causal: { index: '02', eyebrow: 'CAUSAL FIELD', title: '因果场', description: '围绕当前提交展开完整祖先与后继路径，回答“它从哪里来、会影响哪里”。', Icon: Path },
  modules: { index: '03', eyebrow: 'MODULE IMPACT', title: '模块影响', description: '把提交聚合成模块热区，识别变更扩散、协作边界与高频耦合区域。', Icon: TreeStructure },
  risk: { index: '04', eyebrow: 'RISK PATH', title: '风险路径', description: '按变更规模、删除比例、合并复杂度与模块跨度排列需要优先检查的提交。', Icon: Warning },
} satisfies Record<AppMode, { index: string; eyebrow: string; title: string; description: string; Icon: typeof ClockCounterClockwise }>;
...
function CausalPanel({ commits, selected, onSelect }: { commits: GitCommit[]; selected: GitCommit; onSelect: (hash: string) => void }) {
  ...
  const group = (label: string, items: GitCommit[], empty: string) => <div className="causal-group"><span>{label}</span>{items.map((commit) => <button key={commit.hash} onClick={() => onSelect(commit.hash)}><code>{commit.shortHash}</code><b>{commit.subject}</b></button>)}{!items.length && <small>{empty}</small>}</div>;
...
function ModulePanel({ commits, activeModule, onModule }: { commits: GitCommit[]; activeModule: string; onModule: (name: string) => void }) {
  ...
  return <div className="mode-detail module-detail"><button className={activeModule === '全部模块' ? 'active' : ''} onClick={() => onModule('全部模块')}><span>全部模块</span><strong>{modules.length}</strong><small>清除模块筛选</small><i style={{ width: '100%' }} /></button>{modules.map((module) => <button key={module.name} className={activeModule === module.name ? 'active' : ''} onClick={() => onModule(module.name)}><span>{module.name}</span><strong>{compactNumber(module.churn)}</strong><small>{module.commits} 次触达 · 风险 {module.averageRisk}</small><i style={{ width: `${Math.max(10, module.churn / max * 100)}%` }} /></button>)}</div>;
}
...
export function ModeWorkspace({ mode, data, commits, selected, activeModule, onModule, onSelect }: { mode: AppMode; data: RepositoryData; commits: GitCommit[]; selected: GitCommit; activeModule: string; onModule: (name: string) => void; onSelect: (hash: string) => void }) {
  ...
  const metrics = mode === 'history'
    ...
    : mode === 'causal'
      ...
      : mode === 'modules'
        ? [['模块总数', modules.length, '当前范围'], ['热点模块', modules[0]?.name || '—', '按变更量'], ['跨模块提交', commits.filter((commit) => Object.keys(commit.modules).length > 2).length, '涉及 3+ 模块'], ['模块触达', modules.reduce((sum, module) => sum + module.commits, 0), '累计次数']]
