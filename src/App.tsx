...
export default function App() {
  ...
  return <main className="app"><div className="titlebar" />
    <nav className={`topbar active-mode-${mode}`}><div className="logo"><img src="./git-atlas-mark.png" alt="" /><span><strong>Git Atlas</strong><small>仓库情报工作台</small></span></div><div className="mode-tabs">{([['history', ClockCounterClockwise], ['causal', Path], ['modules', TreeStructure], ['risk', Warning]] as const).map(([value, Icon]) => <button key={value} aria-pressed={mode === value} className={mode === value ? 'active' : ''} onClick={() => switchMode(value)}><Icon /><span>{modeCopy[value].title}</span></button>)}</div><div className="density-control"><span>列表密度</span><div role="group" aria-label="列表密度">{([['compact','紧凑'],['standard','标准'],['relaxed','宽松']] as const).map(([value,label]) => <button key={value} aria-pressed={density === value} className={density === value ? 'active' : ''} onClick={() => setDensity(value)}>{label}</button>)}</div></div>{mode === 'causal' ? <label className="causal-toggle"><input type="checkbox" checked={causalOnly} onChange={(event) => setCausalOnly(event.target.checked)} /><i />只看关联路径</label> : <span className="mode-context">{mode === 'history' ? '时间序列' : mode === 'modules' ? activeModule : '中高风险'}</span>}<time>{todayLabel}</time></nav>
