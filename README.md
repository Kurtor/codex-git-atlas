...
<p align="center">
  <strong>把分支图、合并推演与 Codex 任务证据放进同一个本地工作台。</strong><br>
  Git 证明真正落盘了什么，虚拟合并提前暴露冲突，Codex 补足为什么改与验证到哪里；所有证据留在本机。
</p>
...
![Git Atlas 合并推演舱](docs/assets/git-atlas-merge-simulation.png)

<p align="center"><sub>选择源分支与目标分支后，在不 checkout、不移动 Git 引用的前提下完成虚拟合并、冲突检测、变更雷达、验证闸门和只读 Codex 复核。</sub></p>
...
v2.0 没有推翻原来的 Git 图谱，而是在 Git 图谱与 AI 协作层之上增加“合并推演舱”。它不是第三套孤立界面：分支与 Merge Base 来自 Git，任务目标与验证记录来自现有 AI 证据层，检查完成后可以直接回到原来的 Git 操作台。顶部“AI 协作 / Git 图谱”仍可随时切换，两层共享同一个仓库和历史。

| 能力 | 带来的结果 |
| --- | --- |
| 合并推演舱 | 使用 `git merge-tree` 只读预演源分支合入目标分支，显示 Merge Base、领先/落后、潜在冲突、变更雷达和待合并提交序列 |
| 合并闸门 | 把虚拟合并、目标分支新鲜度、工作区状态、Codex 验证记录与任务债务组成可追溯检查单，不再只给一个模糊分数 |
| Codex 分支复核 | 主动调用本机 Codex CLI 在只读沙盒中评审整条分支，输出行为变化、人工检查点、缺少验证和最小回滚边界 |
...
5. 准备合并分支时点击顶部“合并推演”，选择源分支与目标分支；确认检查单后再进入 Git 操作台。
...
顶部“合并推演”是 Git 图谱与 AI 证据之间的决策层。选择源分支和目标分支后，Git Atlas 会执行真实的只读分支计算：

- `git merge-base` 定位共同祖先。
- `git rev-list --left-right --count` 计算源分支领先与落后数量。
- `git diff` 生成待合并文件、模块分布与增删规模。
- `git merge-tree --write-tree --name-only --messages` 进行虚拟三方合并并提取潜在冲突；不会 checkout、merge、rebase 或移动任何引用。
- 工作区、虚拟合并、分支新鲜度、Codex 验证和任务债务组成五道合并闸门；没有证据时显示“未知”，分支完全一致时显示 `SYNCED / 分支无需合并`。
- “让 Codex 复核分支”只在用户点击后运行，通过临时只读会话检查行为变化、人工检查位置、验证缺口与回滚边界。
- “复制合并检查单”可以把所有 Git 结论与证据状态导出为可粘贴的 Markdown；“打开 Git 操作”返回既有白名单操作台。
...
- 合并推演只在用户打开或点击“重新推演”时执行，不轮询分支；所有 Git 子进程完成后立即退出。
...
v2.0.0 加入合并推演、真实冲突检测、合并闸门和 Codex 分支复核，没有新增运行依赖、轮询或后台服务。生产前端产物约为 393.4 KB JavaScript（gzip 111.7 KB）与 82.1 KB CSS（gzip 16.2 KB）；推演引擎按需调用本机 Git，测试覆盖同步分支与真实冲突分支两种结果。
...
- 合并推演使用本机 Git 虚拟合并，只生成计算所需的临时 Git 对象，不修改工作区、分支、索引或引用。
...
npm run test:merge-preflight # 在真实临时分支中验证同步、分叉与冲突识别
