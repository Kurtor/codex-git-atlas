...
默认工作区。按时间扫描稳定 Git DAG、提交信息、模块热度、变更规模、作者与时间；顶部显示提交范围、分支、贡献者和代码变更总量。
...
根据变更规模、删除比例、合并复杂度和模块跨度生成 0–100 风险评分。风险队列列出优先检查项，主列表只保留中高风险提交，并明确展示每条提交的评分与等级。
...
v1.3.0 在 Windows x64、16 逻辑处理器、RAGflow 仓库下的便携版实测：可见空闲工作集约 360.3 MB、私有内存约 294.3 MB、整机 CPU 约 0.02%；最小化后工作集约 352.6 MB、私有内存约 283.9 MB、CPU 采样为 0%。不同仓库规模与系统环境下会有差异，Electron 基线内存也不应被包装成“原生级轻量”。
