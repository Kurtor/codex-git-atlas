...
v1.8.0 将常驻左右栏改为按需抽屉与浮层，并把四层历史样式收敛为单一 token 化视觉系统。没有新增运行依赖、后台任务或动画库；生产前端产物为 336.7 KB JavaScript（gzip 97.3 KB）与 44.1 KB CSS（gzip 8.9 KB），样式体积相较 v1.7.0 下降约 38%。
